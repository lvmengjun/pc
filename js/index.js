(function(){
    ...
    var a=shopping.getElementsByTagName("a");
    document.body.onmouseover = function (e) {
        e = e || window.event;
        e.target = e.target || e.srcElement;
        if (e.target.id === "shopping"|| (e.target.tagName.toLowerCase() === "a" && e.target.parentNode.id === "shopping")) {
            if (mark.style.display === "none") {
                mark.style.display = "block";
                utils.addClass(shopping,'shoppingBg');
                utils.addClass(a,'shoppingBg');

            } else {
                mark.style.display = "none";
                utils.removeClass(shopping,'shoppingBg');
                utils.removeClass(a,'shoppingBg');
            }
            return;
        }
        if (e.target.id === "mark") {
...
(function () {
    ...
    autoTimer = window.setInterval(autoMove, 2000);
    var ulNav = document.getElementById("ulNav"), liList = ulNav.getElementsByTagName("li"),aList=ulNav.getElementsByTagName("a");
    for (var i=0;i<liList.length;i++){
        var cur=liList[i];
        cur.index=i;
        cur.onmouseover=function(){
            this.className = "bg";
            utils.children(liList[this.index])[1].className = "show";
        };
        cur.onmouseleave=function(){
            this.className = null;
            utils.children(liList[this.index])[1].className = null;
        };
    }
